...
Added
- Implemented batch auto-labeling for entire image folders, enabling automatic annotation of all images 
    without manual      interaction.
- Added debug logging for auto-labeling workflow, indicating image processing status (RUN, SKIP, SAVED).
- Introduced automatic skipping of previously labeled images by checking for existing annotation files (.json).

Changed
- Updated Auto Labeling Run behavior:
    - A single execution now processes all images in the current folder.
    - Manual per-image triggering is no longer required.
- Enforced deterministic processing order:
    load image → run model inference → wait for completion → save annotation

Fixed
- Resolved a race condition where annotation files could be saved before model inference completed.
- Prevented redundant inference on images that already contain annotations.

🔧 Code Changes
anylabeling/views/labeling/widgets/auto_labeling/auto_labeling.py
1. Added utility function to detect existing annotation files

<------------------------------------------>
def has_label_file(self, image_path):
    label_path = os.path.splitext(image_path)[0] + ".json"
    return os.path.exists(label_path)
<------------------------------------------>

2. Refactored run_prediction() to support batch auto-labeling
- Iterates through all images in LabelingWidget.image_list
- Skips images that already have annotation files
- Waits for model inference completion before saving labels
- Outputs detailed debug logs for each processing step

<------------------------------------------>
def run_prediction(self):
    view = self.parent
    image_files = view.image_list
    ...
    for image_path in image_files:
        ...
        if self.has_label_file(image_path):
            print(f"SKIP: {image_path}")
            continue

        view.load_file(image_path)
...
📌 Notes
- Existing annotations are detected using LabelMe-style .json files.

- Designed to support large-scale datasets (1,000+ images).

- The implementation is compatible with YOLO-based auto-labeling workflows and can be extended 
    with Resume / Progress indicators in future updates.
