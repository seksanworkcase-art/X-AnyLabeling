...
from PyQt5.QtCore import Qt, QEventLoop ,pyqtSignal, pyqtSlot, QPoint
...
class AutoLabelingWidget(QWidget):
    ...
    def __init__(self, parent):
        ...
        # --- Configuration for: button_stop ---
        self.button_stop = QPushButton("Stop")
        self.button_stop.setShortcut("Shift+I")   # กันชนกับ Run
        ...
        self.button_stop.clicked.connect(self.stop_auto_labeling)
    ...
    def run_prediction(self):
        ...
        for idx, image_path in enumerate(image_files):
            ...
            if self._stop_requested:
                print("🛑 Auto-label stopped by user")
                break

            # ===== SKIP IF LABELED =====
            if self.has_label_file(image_path):
                print(f"⏭️  [{idx+1}/{total}] SKIP (already labeled): {image_path}")
                continue

            try:
                print(f"🔍 [{idx+1}/{total}] RUN: {image_path}")
                ...
                self.model_manager.prediction_finished.connect(on_finished)
                ...
                try:
                    ...
                    self.model_manager.prediction_finished.disconnect(on_finished)
        ...
        print("🎉 Auto-label process finished")
